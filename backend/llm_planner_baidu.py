...
#         logger.info(f"检查代理环境变量: HTTP_PROXY={os.getenv('HTTP_PROXY')}, HTTPS_PROXY={os.getenv('HTTPS_PROXY')}, http_proxy={os.getenv('http_proxy')}, https_proxy={os.getenv('https_proxy')}")
        
#         if proxy_url:
#             logger.info(f"发现代理URL: {proxy_url}")
#             try:
#                 # 支持 socks 代理
#                 if proxy_url.startswith("socks"):
#                     logger.info("使用socks代理")
#                     from httpx_socks import SyncProxyTransport
#                     transport = SyncProxyTransport.from_url(proxy_url)
#                     http_client = httpx.Client(transport=transport)
#                 else:
#                     logger.info("使用http代理")
#                     # 对于同步客户端，使用transport
...
#         else:
#             logger.info("未发现代理配置，使用默认客户端")
#             # 使用默认客户端（会自动检测系统代理，但我们通过transport禁用）
#             http_client = httpx.Client()
