      ...
      <tr onclick="selectRobot('${escapeAttr(r.id)}')">
        ...
        <td style="text-align:right;">
          <div class="actions">
            ...
            <span class="chip" onclick="event.stopPropagation(); alert('更多：详情面板')">更多</span>
          </div>
        </td>
      </tr>
    `
...
async function loadRobots() {
  ...
  const url = `./data/robots.json?t=${Date.now()}`;
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) throw new Error(`加载 robots.json 失败：${res.status}`);
...
async function refresh() {
  try {
    ...
    console.error(e);
    alert(
      "刷新失败：可能是直接用 file:// 打开的页面导致 fetch 被浏览器拦截。\n\n" +
        "请按 README 用本地静态服务器打开（例如：python -m http.server 8000）。"
    );
  }
}
