...
Write-Host "=== Robot Console Dashboard (Unified Port 8000) ===" -ForegroundColor Cyan
...
$fastapi = Start-Process -FilePath $pythonExe -ArgumentList "-m", "uvicorn", "backend.app:app", "--host", "127.0.0.1", "--port", "8000" -PassThru -NoNewWindow
...
Write-Host "Starting Node.js server on port 8000..." -ForegroundColor Green
